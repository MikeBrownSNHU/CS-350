...
class TemperatureMachine(StateMachine):
    ...
    ##
    ##  Immediately update LCD with current mode and set point.
    ##  This helps confirm button presses right away instead of waiting
    ##  for the normal alternating display cycle.
    ##
    def displayStatusNow(self):
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if((counter % 30) == 0):
                serial_data = self.setupSerialOutput()
                print("UART OUT:", serial_data)
...
## Button action is handled in the main loop below for reliable polling.
...
while repeat:
    try:
        greenNow = greenButton.is_pressed
        redNow = redButton.is_pressed
        blueNow = blueButton.is_pressed

        if greenNow and not lastGreen:
            print("Green Button Pressed")
            tsm.processTempStateButton()

        if redNow and not lastRed:
            print("Red Button Pressed")
            tsm.processTempIncButton()

        if blueNow and not lastBlue:
